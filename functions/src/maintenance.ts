...
export const cleanupBehaviorData = onRequest({cors: true}, async (req, res) => {
  if (req.method !== "POST") {
    res.status(405).json({error: "Method not allowed"});
    return;
  }
...
export const cleanupMetrics = onRequest({cors: true}, async (req, res) => {
  if (req.method !== "POST") {
    res.status(405).json({error: "Method not allowed"});
    return;
  }
...
export const checkExpiredSubscriptions = onRequest({cors: true}, async (req, res) => {
  try {
    const now = new Date();
...
export const scheduledSubscriptionCheck = onSchedule({
